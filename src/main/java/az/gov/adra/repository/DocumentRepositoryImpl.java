...
public class DocumentRepositoryImpl implements DocumentRepository {
    ...
    private static final String findDocumentsByKeywordSql = "select id, title , file_url, date_of_reg from Document where title like ? and status = ? order by date_of_reg";
    ...
    public List<DocumentDTO> findAllDocuments(int offset) {
        List<DocumentDTO> documents = jdbcTemplate.query(findAllDocumentsSql, new Object[]{DocumentConstants.DOCUMENT_STATUS_ACTIVE, offset, DocumentConstants.DOCUMENT_FETCH_NEXT_NUMBER}, new ResultSetExtractor<List<DocumentDTO>>() {
            @Override
    ...
    public List<DocumentDTO> findDocumentsByKeyword(String keyword) {
        List<DocumentDTO> documents = jdbcTemplate.query(findDocumentsByKeywordSql, new Object[]{"%" + keyword + "%", DocumentConstants.DOCUMENT_STATUS_ACTIVE}, new ResultSetExtractor<List<DocumentDTO>>() {
            @Override
            public List<DocumentDTO> extractData(ResultSet rs) throws SQLException, DataAccessException {
                List<DocumentDTO> list = new LinkedList<>();
                while (rs.next()) {
