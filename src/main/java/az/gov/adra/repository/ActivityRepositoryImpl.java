...
public class ActivityRepositoryImpl implements ActivityRepository {
    ...
    private static final String deleteActivityByActivityIdSql = "update Activity set status = ? where id = ? and employee_id = ?";
    ...
    //private sqls
    private static final String findCountOfActivitiesByIdSql = "select count(*) as count from Activity where id = ? and status = ?";
...
//    }
    ...
    public void isActivityExistWithGivenId(int id) throws ActivityCredentialsException {
        int count = jdbcTemplate.queryForObject(findCountOfActivitiesByIdSql, new Object[] {id, ActivityConstants.ACTIVITY_STATUS_ACTIVE}, Integer.class);
        if (count <= 0) {
            throw new ActivityCredentialsException(MessageConstants.ERROR_MESSAGE_ACTIVITY_NOT_FOUND);
        }
    }
...
//    }
    ...
    public void deleteActivityByActivityIdAndEmployeeId(Activity activity) throws ActivityCredentialsException {
        int affectedRows = jdbcTemplate.update(deleteActivityByActivityIdSql, ActivityConstants.ACTIVITY_STATUS_INACTIVE, activity.getId(), activity.getEmployee().getId());
        if (affectedRows == 0) {
            throw new ActivityCredentialsException(MessageConstants.ERROR_MESSAGE_INTERNAL_ERROR);
        }
    }
